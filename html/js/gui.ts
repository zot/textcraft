...
export async function editWorld(worldName: string) {
    ...
    async function preventAbort() {
        ...
        if (nameField.value !== world.name) return `World name will not change from ${world.name} to ${nameField.value}`
        for (const childDiv of userList.children) {
            const user = childDiv.originalUser
            const childNameField = $find(childDiv, '[name=mud-user-name]')
            const passwordField = $find(childDiv, '[name=mud-user-password]')
...
function userItem(world: model.World, user: any, processUsersFunc) {
...
function showMudState(state: MudState) {
